...
func NewShowResponseBody(res *approvalviews.ApprovalRequestView) *ShowResponseBody {
	...
	if res.Subject != nil {
		body.Subject = marshalApprovalviewsApprovalSubjectToApprovalSubject(res.Subject)
	}
	return body
}
...
func NewShowResponseBodyExtended(res *approvalviews.ApprovalRequestView) *ShowResponseBodyExtended {
	...
	if res.Parameters != nil {
		...
		for key, val := range res.Parameters {
			tk := key
			body.Parameters[tk] = marshalApprovalviewsParameterViewToParameterResponseBody(val)
		}
	}
	if res.Options != nil {
		body.Options = make([]*ConfigurationOptionResponseBody, len(res.Options))
		for i, val := range res.Options {
			body.Options[i] = marshalApprovalviewsConfigurationOptionViewToConfigurationOptionResponseBody(val)
		}
	}
	if res.ApprovedBy != nil {
		body.ApprovedBy = marshalApprovalviewsUserViewToUserResponseBody(res.ApprovedBy)
	}
	if res.DeniedBy != nil {
		body.DeniedBy = marshalApprovalviewsUserViewToUserResponseBody(res.DeniedBy)
	}
	return body
}
...
func NewIndexOKResponseBody(res *approvalviews.ApprovalRequestListView) *IndexOKResponseBody {
	...
	if res.Items != nil {
		...
		for i, val := range res.Items {
			body.Items[i] = marshalApprovalviewsApprovalRequestViewToApprovalRequestResponseBody(val)
		}
	}
	return body
}
...
func NewIndexOKResponseBodyExtended(res *approvalviews.ApprovalRequestListView) *IndexOKResponseBodyExtended {
	...
	if res.Items != nil {
		...
		for i, val := range res.Items {
			body.Items[i] = marshalApprovalviewsApprovalRequestViewToApprovalRequestResponseBodyExtended(val)
		}
	}
	return body
}
...
func NewApprovePayload(body *ApproveRequestBody, projectID uint, approvalRequestID string, apiVersion string, token *string) *approval.ApprovePayload {
	...
	if body.Options != nil {
		...
		for i, val := range body.Options {
			v.Options[i] = unmarshalConfigurationOptionCreateTypeRequestBodyToApprovalConfigurationOptionCreateType(val)
		}
	}
...
// ValidateApprovalRequestResponseBodyCollection runs the validations defined
// on ApprovalRequestResponseBodyCollection
func ValidateApprovalRequestResponseBodyCollection(body ApprovalRequestResponseBodyCollection) (err error) {
	for _, e := range body {
		if e != nil {
...
// ValidateApprovalRequestResponseBodyExtendedCollection runs the validations
// defined on ApprovalRequestResponseBodyExtendedCollection
func ValidateApprovalRequestResponseBodyExtendedCollection(body ApprovalRequestResponseBodyExtendedCollection) (err error) {
	for _, e := range body {
		if e != nil {
			if err2 := ValidateApprovalRequestResponseBodyExtended(e); err2 != nil {
				err = goa.MergeErrors(err, err2)
			}
		}
	}
